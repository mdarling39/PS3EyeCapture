...
int OCVCapture::retry_ioctl(int request, void* argument)
{
	...
	do
	{
		...
		// If we fail to grab the frame from the buffer ("Resource temporarily unavailable")
		...
		"Trying to request new buffers to make the camera start "
			...
			uint32_t bufferCount = 16; // make sure this matches desiredBuffers

			request.count = bufferCount;
			request.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
			request.memory = V4L2_MEMORY_MMAP;

			// Request new buffers up to 100000 times or until successful
			uint32_t subresult, iters=0;
...
bool OCVCapture::open()
{
	...
	cout << "Camera handle: " << m_camera_handle << endl;
	...
	static const uint32_t desiredBuffers = 16;
